...
describe('WorkerService.selfRegister', () => {
  let service;
  ...
  it('hashes the password and creates an inactive worker with the given name/email', async () => {
    const created = { id: 1, name: 'Alice', email: 'alice@example.com', is_active: false };
    passwordUtilMock.hashPassword.mockResolvedValue('hashed-secret');
    workerRepositoryMock.create.mockResolvedValue(created);

    const result = await service.selfRegister({ name: 'Alice', email: 'alice@example.com', password: 'secret' });
  ...
  it('throws ConflictError with EMAIL_ALREADY_REGISTERED when the email is already taken', async () => {
    ...
    await expect(
      service.selfRegister({ name: 'Alice', email: 'alice@example.com', password: 'secret' })
