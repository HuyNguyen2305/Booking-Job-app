...
export class WorkerRepository extends BaseRepository {
  ...
  /**
   * For each of `workerIds`: whether they have an active booking overlapping
   * [start, end), and their total active-booking hours within [dayStart, dayEnd).
   * Workers with zero bookings still appear (has_overlap: false, booked_hours: 0).
   */
  async getAvailability(workerIds, { start, end, dayStart, dayEnd }, { transaction } = {}) {
    ...
    const rows = await this.model.sequelize.query(
      ...
      SELECT
        worker_id,
        BOOL_OR(status IN (:activeStatuses) AND start_time < :end AND end_time > :start) AS has_overlap,
        COALESCE(SUM(
          CASE WHEN status IN (:activeStatuses) AND start_time < :dayEnd AND end_time > :dayStart
      ...
      `,
      {
        replacements: { workerIds, start, end, dayStart, dayEnd, activeStatuses: ACTIVE_BOOKING_STATUSES },
